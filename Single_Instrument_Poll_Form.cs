...
namespace Multimeter_Controller
{
  public partial class Single_Instrument_Poll_Form : Form
  {
    ...
    private void Apply_Settings ( )
    {
      ...
      NPLC_Combo.Enabled = ( _Selected_Meter == Meter_Type.HP3458 );
  ...
  _Readings.Count, _Enable_Rolling, _Max_Display_Points, _View_Offset );
    ...
    private void Populate_Function_Combo ( )
    {
      ...
      {
        string Cmd = _Selected_Meter == Meter_Type.HP34401
          ? _Functions [ I ].Cmd_34401
          : _Functions [ I ].Cmd_3458;
    ...
    private async void Start_Reading ( )
    {
      ...
      string Configure_Cmd = _Selected_Meter == Meter_Type.HP34401
          ? Selected.Cmd_34401 : Selected.Cmd_3458;
      ...
      try
      {
        if ( !Is_Query_Mode )
        {
          ...
          else if ( _Selected_Meter == Meter_Type.HP34401 )
          {
  ...
  _Readings.Count, _Enable_Rolling, _Max_Display_Points, _View_Offset );
    ...
    // to display the full 8.5 digits of precision from your HP3458 meter.
    //
    ...
    private static string Format_Value ( double Value, string Unit, Meter_Type Meter = Meter_Type.HP34401 )
    {
      using var Block = Trace_Block.Start_If_Enabled ( );
      ...
      if ( Abs >= 1.0 )
        return Is_HP
            ? $"{Value:F10} {Unit}"   // 10 digits for 3458
            : $"{Value:F8} {Unit}";   // 8.5 digit for 34401
      if ( Abs >= 0.001 )
        return Is_HP
  ...
  _Readings.Count, _Enable_Rolling, _Max_Display_Points, _View_Offset );
    ...
    private void NPLC_Button_Click ( object Sender, EventArgs E )
    {
      ...
      if ( _Selected_Meter != Meter_Type.HP3458 )
      {
        MessageBox.Show ( "NPLC is only applicable to the 3458.",
      ...
      try
      {
        switch ( _Selected_Meter )
        {
          ...
          case Meter_Type.HP3458:
            ...
            // LOCAL is not a valid written command on the 3458
