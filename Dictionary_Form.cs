...
namespace Multimeter_Controller
{
  public partial class Dictionary_Form : Form
  {
    ...
    public Dictionary_Form( Meter_Type Meter = Meter_Type.HP3458 )
    {
      InitializeComponent();

      string Meter_Name = Meter switch { Meter_Type.HP34401 => "HP34401",
    ...
    private void Populate_Category_Filter()
    {
      ...
      foreach ( Command_Category Category in Enum.GetValues( typeof( Command_Category ) ) )
      {
    ...
    {
      string Search_Text       = Search_Box.Text.Trim().ToUpperInvariant();
      string Selected_Category = Category_Filter_Combo.SelectedItem?.ToString() ?? "All Categories";

      _Filtered_Commands = _All_Commands
                             .Where( Cmd =>
                                     {
                                       bool Category_Match = Selected_Category == "All Categories" ||
                                       ...
                                       return Cmd.Command.ToUpperInvariant().Contains( Search_Text ) ||
                                              ...
                                              Cmd.Description.ToUpperInvariant().Contains( Search_Text ) ||
                                              Cmd.Parameters.ToUpperInvariant().Contains( Search_Text );
                                     } )
                             .ToList();

      Status_Label.Text = $"{_Filtered_Commands.Count} of {_All_Commands.Count} commands";
      Bind_Grid();
    }

    private void Search_Box_Text_Changed( object Sender, EventArgs E )
    {
      Apply_Filter();
    }

    private void Category_Filter_Combo_Selected_Index_Changed( object Sender, EventArgs E )
    {
      Apply_Filter();
    }
    ...
    private void Command_Grid_Cell_Double_Click( object Sender, DataGridViewCellEventArgs E )
    {
      if ( E.RowIndex < 0 || E.RowIndex >= _Filtered_Commands.Count )
      {
        return;
      }

      Command_Entry Selected_Command = _Filtered_Commands[ E.RowIndex ];
      string        Detail_Text =
