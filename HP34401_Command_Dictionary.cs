...
namespace Multimeter_Controller
{
  public static class HP34401_Command_Dictionary_Class
  {
    ...
    public class Test_Profile : IInstrument_Test_Profile
    {
      ...
      public List<Command_Entry> Get_Commands() => HP34401_Command_Dictionary_Class.Get_All_Commands();
      ...
      public static List<Command_Test_Result> Test_All_Commands(
        Func<string, string> Query, // wraps _Comm.Query_Instrument
        Action<string>       Send,  // wraps _Comm.Send_Instrument_Command
        Action<string>? Progress = null )
      {
        ...
        {
          Progress?.Invoke( $"Testing: {Cmd.Command}" );
          Command_Test_Result Result;

          try
          {
            Result = Cmd.Test_Behavior switch { Test_Behavior.Query_Safe => Run_Query( Cmd, Query ),
                                                Test_Behavior.Write_Then_Query =>
                                                  Run_Write_Then_Query( Cmd, Send, Query ),
                                                Test_Behavior.Requires_Sequence =>
                                                  Command_Test_Result.Skip( Cmd, "Requires sequence" ),
                                                Test_Behavior.Skip_Destructive =>
                                                  Command_Test_Result.Skip( Cmd, "Destructive — skipped" ),
          ...
          // Check error queue after every non-skipped command
          if ( ! Result.Skipped )
          {
            try
            {
              string Err = Query( "SYST:ERR?" );
        ...
        // Run the sequenced blocks
        Results.AddRange( Test_Fetch_Sequence( Query, Send ) );
        Results.AddRange( Test_Bus_Trigger_Sequence( Query, Send ) );
        Results.AddRange( Test_Math_Average_Sequence( Query, Send ) );
      ...
      private static Command_Test_Result Run_Query( Command_Entry Cmd, Func<string, string> Query )
      {
        string Target = ! string.IsNullOrWhiteSpace( Cmd.Query_Form ) ? Cmd.Query_Form : Cmd.Command;

        string Response = Query( Target );

        return string.IsNullOrWhiteSpace( Response ) ? Command_Test_Result.Fail( Cmd, "Empty response" )
      ...
      {
        // Skip write-then-query if there's no query form to verify against
        if ( string.IsNullOrWhiteSpace( Cmd.Query_Form ) )
          return Command_Test_Result.Skip( Cmd, "No query form to verify" );

        // Send the example value from the dictionary entry
