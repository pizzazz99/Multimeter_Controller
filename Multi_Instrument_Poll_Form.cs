    ...
    */
      ...
      try
      {
        ...
        while (!Token.IsCancellationRequested && (Continuous || _Cycle_Count < Total_Cycles))
        {
          ...
          for (int I = 0; I < _Series.Count; I++)
          {
            ...
            Capture_Trace.Write( $"Cycle {_Cycle_Count} | Series[{I}].Points.Count = {S.Points.Count}" );
    ...
    private void Clear_Button_Click( object Sender, EventArgs E )
    {
      ...
      if (_Data_From_File && _Original_Series.Count > 0)
      {
        ...
        _Series.AddRange( _Original_Series );
        _Original_Series.Clear();
      }
      ...
      Delay_Numeric.Value =
        Math.Max( Delay_Numeric.Minimum, Math.Min( Delay_Numeric.Maximum, Calculate_Nplc_Min_Ms() ) );
    ...
    private static void Append_GPU_To_Settings( string File_Path, Snapshot GPU_Start, Snapshot GPU_End )
    {
      ...
      try
      {
        ...
        W.WriteLine(
          $"  Duration : {GPU_Snapshot.Format_Duration( Duration )} ({Duration.TotalSeconds:F1}s)" );
        W.WriteLine( "=============================================================" );
                                                ...
                                                Snapshot? GPU_Start = null )
    {
      ...
      try
      {
        ...
        W.WriteLine(
          $"  Auto Save          : {(Settings.Enable_Auto_Save ? $"Every {Settings.Auto_Save_Interval_Minutes} min" : "Disabled")}" );
        ...
        W.WriteLine(
          $"  Auto Trim Old Data : {(Settings.Auto_Trim_Old_Data ? $"Keep last {Settings.Keep_Last_N_Points:N0}" : "Disabled")}" );
        W.WriteLine();

        // ── Performance ───────────────────────────────────────────
        W.WriteLine( "[Performance]" );
        W.WriteLine(
          $"  Rendering Engine   : {(Settings.Use_GPU_Rendering && Settings.GPU_Rendering_Available ? "GPU (SkiaSharp/OpenGL)" : Settings.Use_GPU_Rendering && !Settings.GPU_Rendering_Available ? "CPU (GPU requested but unavailable)" : "CPU (GDI+)")}" );
        W.WriteLine( $"  Max Points Memory  : {Settings.Max_Data_Points_In_Memory:N0}" );
        W.WriteLine( $"  Warning Threshold  : {Settings.Warning_Threshold_Percent}%" );
        W.WriteLine( $"  Warn At Threshold  : {Settings.Warn_At_Threshold}" );
        W.WriteLine(
          $"  Throttle Refresh   : {(Settings.Throttle_When_Many_Points ? $"Yes — above {Settings.Throttle_Point_Threshold:N0} points" : "No")}" );
        W.WriteLine( $"  Reduce Refresh Rate: {Settings.Reduce_Refresh_Rate_When_Large}" );
        W.WriteLine(
          $"  Auto Trim          : {(Settings.Auto_Trim_Old_Data ? $"Yes — keep last {Settings.Keep_Last_N_Points:N0}" : "No")}" );
        W.WriteLine( $"  Chart Decimation   : {(Settings.Enable_Decimation ? "Enabled" : "Disabled")}" );
        W.WriteLine(
          $"  Decimate Above     : {(Settings.Enable_Decimation ? $"{Settings.Decimation_Threshold:N0} points" : "N/A")}" );
        W.WriteLine(
          $"  Sample Every N Pts : {(Settings.Enable_Decimation ? $"{Settings.Decimation_Step} points" : "N/A")}" );
        W.WriteLine();
        ...
        W.WriteLine(
          $"  Default IP         : {(string.IsNullOrEmpty( Settings.Default_IP_Address ) ? "Auto-detect" : Settings.Default_IP_Address)}" );
    ...
    private void Run_Auto_Analysis_If_Enabled( bool Force = false )
    {
      ...
      var Popup =
        new Analysis_Popup_Form( Instruments, _Theme ?? Chart_Theme.Dark_Preset(), _Settings, Master_Name );
      Popup.Show( this );
      Popup.Begin_Async_Load();
    }
    ...
    private void Chart_Panel_Paint( object? sender, PaintEventArgs e )
    {
      ...
      if (_Settings.Enable_Decimation)
      {
        ...
        if (Total > _Settings.Decimation_Threshold)
        {
          ...
          G.FillRectangle( Pill_Brush, Warn_X - 4, Warn_Y - 2, Warn_Size.Width + 8, Warn_Size.Height + 4 );
    ...
    private void Chart_Theme_Button_Click( object Sender, EventArgs E )
    {
      ...
      if (Dlg.ShowDialog( this ) == DialogResult.OK)
      {
        ...
        _Settings.Set_Theme( _Theme, "Chart Theme" );  // ← fires Theme_Changed

        // Theme_Changed handler takes care of the rest, but if you want
        // to be explicit here too:
        Chart_Panel.BackColor = _Theme.Background;
        for (int I = 0; I < _Series.Count; I++)
          _Series[ I ].Line_Color = _Theme.Line_Colors[ I % _Theme.Line_Colors.Length ];
    ...
    private void NPLC_Summary_Button_Click( object Sender, EventArgs E )
    {
      ...
      if (_NPLC_Summary_Popup == null || _NPLC_Summary_Popup.Form.IsDisposed)
      {
        ...
        foreach (var S in _Series)
        {
          ...
          _NPLC_Summary_Popup.Add_Heading_Mono( $"{S.Name}  GPIB: {S.Address}" )
    ...
    private void GPU_Monitor_Button_Click( object sender, EventArgs e )
    {
      ...
      if (_GPU_Monitor.Is_Open)
      {
        ...
        GPU_Monitor_Button.BackColor = SystemColors.Control;
      }
      else
      {
    ...
    private void Memory_Monitor_Button_Click( object sender, EventArgs e )
    {
      ...
      if (_Memory_Monitor.Is_Open)
      {
        ...
        Memory_Monitor_Button.BackColor = SystemColors.Control;
      }
      else
      {
