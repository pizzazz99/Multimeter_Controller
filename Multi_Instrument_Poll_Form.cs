...
namespace Multimeter_Controller
{
  ...
  S.Points.Count, _Enable_Rolling, _Max_Display_Points, _View_Offset );
    ...
    private async void Start_Polling ( )
    {
      ...
      string Configure_Cmd = _Series.Count > 0 && _Series [ 0 ].Type == Meter_Type.HP34401A
          ? Selected.Cmd_34401A
          : Selected.Cmd_3458A;
      ...
      try
      {
        ...
        for ( int I = 0; I < _Series.Count; I++ )
        {
          ...
          {
            string NPLC_Cmd = S.Type == Meter_Type.HP34401A
          ...
          if ( S.Type == Meter_Type.HP3458A )
          {
            ...
            Capture_Trace.Write ( $"Priming 3458A..." );
          ...
          else if ( S.Type == Meter_Type.HP34401A )
          {
            ...
            Capture_Trace.Write ( $"Sending CONF command: {Conf_Cmd} to HP34401A" );
               ...
               ( Continuous || _Cycle_Count < Total_Cycles ) )
        {
          ...
          for ( int I = 0; I < _Series.Count; I++ )
          {
            ...
            if ( S.Type == Meter_Type.HP3458A )
            {
              Capture_Trace.Write ( $"║  3458A: ++read eoi" );
              await Task.Run ( ( ) => _Comm.Raw_Write_Prologix ( "++read eoi" ), Token );
            ...
            {
              Capture_Trace.Write ( $"║  34401A: READ?" );
              Response = await Task.Run ( ( ) =>
    ...
    private string Get_Command_For_Series ( Instrument_Series S,
      (string Label, string Cmd_3458A, string Cmd_34401A, string Cmd_Generic_GPIB, string Unit) Entry )
    {
      return S.Type switch
      {
        Meter_Type.HP3458A => Entry.Cmd_3458A,
        Meter_Type.HP34401A => Entry.Cmd_34401A,
  ...
  Brush Label_Brush )
    ...
    private string Format_Value ( double Value )
    {
      ...
      {
        return Value.ToString ( "F10" ); // 0.0000123456
      }
    }
  ...
  ref _Memory_Warning_Shown );
    ...
    */
    ...
    private string old_Get_Measurement_Command ( Meter_Type Type )
    {
      int Filtered_Index = _Filtered_Indices [ Measurement_Combo.SelectedIndex ];
      var Entry = _Measurements [ Filtered_Index ];
      return Type == Meter_Type.HP3458A
          ? Entry.Cmd_3458A
          : Entry.Cmd_34401A;
    }
...
Pen Grid_Pen, Font Label_Font, Brush Label_Brush )
  ...
  _Max_Display_Points, _Auto_Scroll, ref _View_Offset );
    ...
    private void Populate_Measurement_Combo ( )
    {
      ...
      {
        string Cmd = _Series.Count > 0 && _Series [ 0 ].Type == Meter_Type.HP34401A
            ? _Measurements [ I ].Cmd_34401A
            : _Measurements [ I ].Cmd_3458A;
        if ( !string.IsNullOrEmpty ( Cmd ) )
        {
