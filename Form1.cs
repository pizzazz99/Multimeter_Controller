  ...
  public partial class Form1 : Form
  {
          ...
          if (_Comm.Is_Address_Verified( Address ))
          {
            ...
            if (Detected != Meter_Type.Generic_GPIB && Detected != Type)
            {
              ...
              if (Result == DialogResult.Yes)
              {
                ...
                {
                  MessageBox.Show(
                      $"The instrument name has been updated to match the detected type.\n\n" +
                      $"  Selected:  {Selected_Name}\n" +
    ...
    private async Task Release_Instruments_To_Local_Async()
    {
      foreach (var S in _Instruments)
      {
        ...
        catch { }  // don't let one instrument block the others
      }
    }
    ...
    private void Meter_Info_Button_Click( object Sender, EventArgs E )
    {
      ...
      using var Popup = new Rich_Text_Popup( "Supported Meter Types", 660, 600, Resizable: true );
                                           ...
                                           Chart_Theme Theme )
    {
      ...
      Popup.Add_Blank()
           ...
           .Add_Row( "Auto Save", Settings.Enable_Auto_Save
                                                  ? $"Every {Settings.Auto_Save_Interval_Minutes} min"
                                                  : "Disabled" )
      ...
      Popup.Add_Blank()
           ...
           .Add_Row( "Auto Trim Old Data", Settings.Auto_Trim_Old_Data
                                                  ? $"Keep last {Settings.Keep_Last_N_Points:N0}"
                                                  : "Disabled" )
           .Add_Blank();
      ...
      Popup.Add_Blank()
           ...
           .Add_Row( "Default IP", string.IsNullOrEmpty( Settings.Default_IP_Address )
                                                    ? "Auto-detect"
